...
function SearchContent() {
   ...
   // Track if search was initiated manually to skip modal
   ...
   useEffect(() => {
      if (data && url) {
         sessionStorage.setItem('closai_last_analysis', JSON.stringify({
            url,
            data,
            timestamp: Date.now()
         }));
   ...
   useEffect(() => {
      ...
      const savedSession = sessionStorage.getItem('closai_last_analysis');
      let restored = false;
      ...
      if (savedSession) {
         try {
            ...
            // If we have a target URL, only restore if it matches
            if (targetUrl) {
               if (parsed.url === targetUrl && parsed.data) {
                  setUrl(parsed.url);
                  setAnalyzedUrl(parsed.url);
                  setData(parsed.data);
                  setShowShareModal(false); // Don't show modal if we are restoring context (e.g. back button)
                  restored = true;
               }
            }
            // If no target URL (e.g. just /search), restore whatever was last
            // RE-ENABLED: This allows persistence when coming back from Closet
            else if (!targetUrl && parsed.url && parsed.data) {
               // Update URL to match the restored data
               // This satisfies "At /search, only show search bar" by moving away from /search
               router.replace(`/search?url=${encodeURIComponent(parsed.url)}`);

               // We don't strictly need to set state here because the router replace will trigger 
               // the effect again with the new URL, and the FIRST block will handle restoration.
               // BUT to avoid flicker, we can set it. 
               // actually, if we set state here, the next effect run might re-set it.
               // Let's just redirect and let the NEXT run handle the actual data setting?
               // Or set it to show content immediately?
      ...
      // If not restored and we have a target URL, proceed with new analysis flow
      if (!restored && targetUrl) {
         if (targetUrl !== analyzedUrl) { // Only if different
            // Update input if it doesn't match (e.g. back button)
            if (url !== targetUrl) setUrl(targetUrl);

            const source = searchParams.get('source'); // Check source param

            // Only show modal if NOT a manual search AND NOT from internal source (like closet)
            // i.e. ONLY show if came from external share link
            if (!isManualSearch.current && !source) {
               setShowShareModal(true);
            } else {
               setShowShareModal(false);
            }

            // Reset manual flag for next navigation (e.g. back button handling)
            isManualSearch.current = false;

            handleAnalyze(null, targetUrl);
         }
      } else if (!restored && !targetUrl) {
         // If we are at root /search without a URL param, clear the state
   ...
   const handleSearch = (e: React.FormEvent) => {
      e.preventDefault();
      if (!url) return;
      isManualSearch.current = true; // Mark as manual search
      router.push(`/search?url=${encodeURIComponent(url)}`);
   };

   const handleReset = () => {
      sessionStorage.removeItem('closai_last_analysis');
      setData(null);
      setUrl('');
      setAnalyzedUrl('');
      // We are likely already at /search or /search?url=...
      // Just clearing state is enough if we want to show the search bar.
      // But let's fix the URL too.
      router.push('/search');
   };
   ...
   return (
      <div className="min-h-screen bg-white text-black font-sans selection:bg-black selection:text-white pb-24 lg:pb-0">
         ...
         <header className="fixed top-0 left-0 right-0 h-20 bg-white/90 backdrop-blur-xl z-50 flex items-center justify-between px-4 md:px-8 border-b border-neutral-100">
            <button onClick={handleReset} className="text-2xl font-black tracking-tighter hover:opacity-50 transition-opacity">
               ClosAI
            </button>
            <nav className="flex gap-4 md:gap-8">
               {/* Use handleReset for 'SEARCH' to escape persistence */}
               <button onClick={handleReset} className="text-sm font-bold text-black border-b-2 border-black pb-1">SEARCH</button>
               <Link href="/profile" className="text-sm font-medium text-neutral-400 hover:text-black transition-colors">WARDROBE</Link>
               <Link href="/share-guide" className="text-neutral-400 hover:text-black transition-colors" title="iOS Share Guide">
                  <Info size={20} />
               </Link>
